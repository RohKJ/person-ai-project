...
```text
CSV Generator -> SQLite DB -> Analysis Tools <- Tool Registry
                                      ^               ^
                                      |               |
...
Agent는 OpenAI API Key 없이 동작하는 Mock 모드와 OpenAI Responses API Tool Calling 모드를 지원합니다.
구조는 `Agent Service -> Provider -> Tool Registry -> Tool Args Schema -> Analysis Function -> Answer Formatter` 흐름입니다.

Agent 모드:

| `AGENT_MODE` | 동작 |
| --- | --- |
| `auto` | `OPENAI_API_KEY`가 있으면 OpenAI, 없으면 Mock |
| `mock` | 항상 rule-based Router 사용 |
| `openai` | 반드시 OpenAI Responses API 사용. API Key가 없으면 명확한 오류 반환 |

OpenAI 모드에서도 모델은 Tool 이름과 입력 인자만 선택합니다. 숫자 계산과 최종 답변 문장은 기존 분석 함수와 deterministic formatter가 생성합니다.
...
- `/agent/tools`에서 등록 Tool과 OpenAI Responses API용 strict function schema를 확인할 수 있습니다.
- 향후 LangGraph를 추가해도 Provider, Tool Registry, 분석 함수는 그대로 재사용할 수 있습니다.

## 7. 실행 방법

Python 3.11 기준입니다. Windows PowerShell에서는 아래 간편 스크립트를 사용할 수 있습니다.

환경변수 설정이 필요한 경우 `.env.example`을 참고해 `.env` 파일을 만듭니다.
...
API Key가 없으면 `AGENT_MODE=auto` 상태에서 자동으로 Mock Agent가 사용됩니다.
...
| `GET` | `/agent/status` | 요청 모드, 실제 Provider, API Key 설정 여부 조회 |
...
curl -X POST http://127.0.0.1:8000/agent/query ^
  ...
  -d "{\"query\":\"오늘 슬랙 보고서 만들어줘\",\"mode\":\"auto\"}"
```

OpenAI Tool Calling 구현은 OpenAI 공식 [Function calling guide](https://developers.openai.com/api/docs/guides/function-calling)를 기준으로 합니다.
